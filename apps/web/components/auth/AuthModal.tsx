...
export function AuthModal({
  ...
  subtitle = 'Start tracking interstellar objects',
}: AuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auth successful - call onSuccess callback
      onSuccess()
    } catch (err: unknown) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error) throw error
      // Note: OAuth will redirect, so we don't call onSuccess here
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'An error occurred')
      setIsLoading(false)
    }
      ...
      <div className="relative z-10 w-full max-w-md mx-4 bg-white rounded-xl shadow-2xl">
        ...
        <div className="px-8 pt-8 pb-4 text-center">
          <h2 id="auth-modal-title" className="text-2xl font-bold text-gray-900">
            {mode === 'sign-up' ? title : 'Welcome back'}
          </h2>
          <p className="mt-2 text-sm text-gray-600">
            {mode === 'sign-up' ? subtitle : 'Sign in to your account'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-8 py-6 space-y-4">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 py-3 px-4 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
