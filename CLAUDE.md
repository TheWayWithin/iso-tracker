...
**User Signup Flow**: User records (profile, subscription, notification_preferences) are created in **application code** (`apps/web/app/auth/actions.ts`), NOT via database triggers.

**Why**: Supabase doesn't reliably support custom triggers on `auth.users` table due to permissions and schema isolation. Application-level creation provides better error handling, explicit control flow, and avoids trigger conflicts.

**Critical Rule**: ❌ **NEVER** create database triggers on `auth.users` table. ✅ Always handle auth-related record creation in application layer.
