...
-- DEPRECATED: The trigger below was removed in Migration 011/012 due to Supabase
-- limitations with custom triggers on auth.users table. User record creation
-- (profile, subscription, notification_preferences) is now handled in
-- application code: apps/web/app/auth/actions.ts signUp() function.
--
-- Keeping trigger code here for reference, but it is NOT deployed to database.

-- Auto-create notification preferences on user signup (DEPRECATED - NOT DEPLOYED)
